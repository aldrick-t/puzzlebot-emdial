...
class TLightRecogniLegacy(Node):
    def __init__(self):
        super().__init__('trafficlight_recogni_legacy')
        ...
        self.declare_parameter('min_area_ratio', 0.032)
    ...
    def isolate_shape(self, img, mask, color_name, contour, min_area):
        ...
        if self.feature_type == 'circle':
            ...
            if circle_area < min_area:
                ...
                return img
            self.get_logger().info(f"{color_name} circle @({center[0]},{center[1]}) r={radius} area={circle_area:.1f}", throttle_duration_sec=5.0)
    ...
    def show_mosaic(self, processed, mask_r, mask_y, mask_g, processed_override=None):
        ...
        cv2.imshow('Debug View - TLRecogni Legacy', np.vstack([top,bottom]))
        cv2.waitKey(1)

    def timer_callback(self):
        if not self.image_received_flag:
            self.get_logger().info('Waiting for color...', throttle_duration_sec=10.0)
        self.image_received_flag = False
