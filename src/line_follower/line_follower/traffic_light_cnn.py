...
class TrafficLightCNN(Node):
    ...
    def __init__(self):
        ...
        # Load the model
        self.model = torch.load('/models/traffic_light_model.pth')
        self.model.eval()
    ...
    def wait_for_ros_time(self):
        ...
        while self.get_clock().now().nanoseconds == 0:
            self.get_logger().info("Waiting for ROS time to be set...")
            rclpy.spin_once(self)
            # Sleep for a short duration to avoid busy waiting
            rclpy.sleep(0.1)
        self.get_logger().info("ROS time is set.")
        # Set the time to the current time
        self.get_clock().now().nanoseconds = Time().nanoseconds
