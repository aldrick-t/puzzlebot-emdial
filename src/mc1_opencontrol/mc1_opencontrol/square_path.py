...
from geometry_msgs.msg import Twist  # Import Twist for publishing
import math
import numpy as np
...
class OpenLoopController(Node):
    def __init__(self):
        ...
        # Create a subscription to receive pose messages
        ...
        # Create publisher for cmd_vel (like in square_path.py)
    ...
    def pose_callback(self, msg: Float32MultiArray):
        data = msg.data
        twist_cmd = Twist()  # Create a Twist message to publish the speed commands
        ...
        if self.mode == "coordinate":
            ...
            # Publish the computed speeds to /cmd_vel
            twist_cmd.linear.x = commanded_linear_speed
            twist_cmd.angular.z = commanded_angular_speed
        ...
        # For velocity mode, the message is [v, w, t]
            ...
            # Publish the computed speeds to /cmd_vel
            twist_cmd.linear.x = effective_v
            twist_cmd.angular.z = effective_w
            self.cmd_vel_pub.publish(twist_cmd)
